/*
	BeetlePara
	Utility Servlet to list paralogues to a gene and allow BeetleAtlas Query
	Use TC001965 to test for paras, TC001906 gives 2 orthos, TC001919 gives 6
	DPL 10.12.2023
*/	
...
public class BeetlePara extends HttpServlet
{		
	...
	{	
		...
		if(type.equals("para"))
		{
			...
			String url = "/BeetleAtlas/?search=bulk&geneList=" + id + "," + buildString(paras);
			...
			out.println("<li>Pressing the ‘Go’ button, below, will open a new BeetleAtlas window showing the tissue distribution of transcripts of each paralogue.<br>");
			out.println("<li>We do not provide facilities in BeetleAtlas for comparing the sequences of paralogues. If you wish to do this we suggest that you copy the list below, obtain the protein sequences from iBeetleBase, and compare them using publicly available alignment tools.</li></ul>\n");
		...
		else if(type.equals("ortho"))
		{
			...
			out.println("<li>We do not provide facilities for comparing the fly and beetle sequences. If you wish to do this we suggest that you copy the list below, obtain the protein sequences from iBeetleBase and FlyBase, and compare them using publicly available alignment tools.</li></ul>\n");
			out.println("<p style=\"text-align:center;\"><button onclick=\"openLinkWindow('" + url + "');\">Go</button></p>");
	...
	private String getPara(String geneID)
	{
		...
		ParamQuery parQ = DBQuery.getParamQuery("PARAS_FROM_GENEID");
		try 
		{
	...
	private String getOrtho(String geneID)
	{
		...
		Connect cnt = new Connect();
		Connection conn = cnt.getConnection();
		ParamQuery parQ = DBQuery.getParamQuery("FLY_ORTHOLOGUES_FROM_ID");
		try 
		{
			parQ.setPrepStatement(conn);
			PreparedStatement prepStat = parQ.getPrepStatement();
