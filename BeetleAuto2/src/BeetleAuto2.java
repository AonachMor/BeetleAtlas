...
public class BeetleAuto2 extends HttpServlet
{
	// Number flags as variables to aid comprehension
	private final int NCBI_ID = 0;
	private final int GENE_SYMBOL = 1;
	private final int TC_ID = 2;
	...
	{	
		...
		if(textEntered != null && !textEntered.equals(""))
		{
			...
			if(searchType.equals("ncbiID"))
			{
				mode = NCBI_ID;
				tooSmall = 4;
			}
			else if(searchType.equals("ncbiSymbol"))
			{
				mode = GENE_SYMBOL;
				tooSmall = 1;
			}
			else if(searchType.equals("tcID"))
			{
				mode = TC_ID;
				tooSmall = 5;
			}
	...
	public int doQuery(String partWord, PrintWriter writer)
	{
		...
		{
		 	parQ= DBQuery.getParamQuery("TC_ID");
		}
		...
		try 
		{
			...
			if(mode == PRODUCT)	// expand at start and end of partWord
			{
