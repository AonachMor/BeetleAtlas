...
{	
	...
	{	
		...
		for(int i=0; i<max; i++)
		{
			GeneSearch gs = new GeneSearch(pairList[i].getNCBIid(), "ncbiID", tCat);
			Expression express;
...
/*		if(conn != null)
	...
	{
		// Get list of NCBI_IDs of genes that peak at a specific time in embryo development
		...
		{		
			...
			if(resSet.first())
			{
				...
				{	
					...
					GeneFPKMpair eg = new GeneFPKMpair(ncbiID, fpkm);					
	...
	{
		// Get list of NCBI_IDs of embryo-only genes that peak at a specific time in embryo development
		String embryoQuery = new String();
	...
	// INNER CLASS
	// For storing a ncbiID and FPKM from an embryo search
 	class GeneFPKMpair
 	{
 		String ncbiID;	// ncbiID
 		double fpkm;	// FPKM of peak age
 		
 		GeneFPKMpair(String ncbiID, double fpkm)
 		{
 			this.ncbiID = ncbiID;
 			this.fpkm = fpkm;
 		}
