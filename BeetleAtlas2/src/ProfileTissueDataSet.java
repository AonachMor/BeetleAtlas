...
public class ProfileTissueDataSet 
{
	...
	public ProfileTissueDataSet(String queryGeneID, boolean aleProfile)
	{	
		dataList = new ProfileTissueData [LIST_LENGTH];
		populateList(queryGeneID, aleProfile);
	}
	
	private void populateList(String queryGeneID, boolean aleProfile)
	{
		String query = new String();		// NCBI_ID, FPKM, Status, TissueID for ALL genes
		if(aleProfile)
		{
			query = DBQuery.getProfileALEQuery();
		}
		else
		{
			query = DBQuery.getProfileALQuery();
		}
		...
		try 
		{
			...
			if(resSet.first())
			{
				...
				{	
					if(!resSet.getString(1).equals(queryGeneID))					// if it's not the query data set
					{
						// checks if new set by by initial value of atStart or change in value of ncbiID (String 1)
						if(atStart || !resSet.getString(1).equals(ncbiID))
						{
							...
							// read this first ncbiID and use it to construct the new GeneExCorrelation
							ncbiID = resSet.getString("NCBI_ID");						
							data = new ProfileTissueData(ncbiID);
							...
							ProfileDatum datum = new ProfileDatum(ncbiID, fpkm, status, tissueID);
							data.addDatum(datum);
