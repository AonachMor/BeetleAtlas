...
{	
	...
	{	
		...
		{
			GeneSearch gs = new GeneSearch(develRatioList[i].getNCBIid(), "ncbiID", tCat);
			Expression express;
...
/*		if(conn != null)
		...
		try 
		{
			...
			if(resSet.first())
			{
				...
				{	
					...
					if (ratio > 4)
					{
						DevelRatio dr = new DevelRatio(ncbiID, ratio);					
	...
	// INNER CLASS
 	...
 	class DevelRatio
 	{
 		...
 		DevelRatio(String ncbiID, double ratio)
 		{
 			this.ncbiID = ncbiID;
 			this.ratio = ratio;
 		}
