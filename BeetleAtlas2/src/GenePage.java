...
public class GenePage
{
	...
	{	
		...
		htmlBuilder.append("<input type=\"radio\" name=\"idtype\" id=\"ncbiSymbol\" /> Gene Symbol (e.g. Mhc, LOC660130)<br />\n");	
		htmlBuilder.append("<input type=\"radio\" name=\"idtype\" id=\"product\" /> Gene Product (enter partial term like ‘eye’, ‘inositol’)<br />\n");
		htmlBuilder.append("<input type=\"radio\" name=\"idtype\" id=\"tcID\" /> TC ID (switches modes)<br />\n");
/*		htmlBuilder.append("<a href=\"javascript:window.open('https://motif.mvls.gla.ac.uk/BeetleAtlas/?page=gene','_self');\" "
				+ "title=\"Open BeetleAtlas in OGS3 mode\"><button class=\"fauxRadio\"></button></a> TC ID (switches modes)\n");*/
		...
		htmlBuilder.append("<button onclick=\"sendSearchGeneForm2();\">Search</button>\n</p>\n");
		
		// Switch Button
		htmlBuilder.append("<p style=\"text-align:right;\">\n");
		htmlBuilder.append("<a href=\"https://motif.mvls.gla.ac.uk/BeetleAtlas/?page=gene\">\n");
	...
	{		
		...
		// RESULTS FORMATTED
		if(expression!=null && !gene.isDiscontinued())
		{
		...
		{	
			...
			{
				if(gene.getLocus().equals("na"))
				{
					htmlBuilder.append("Gene ‘" + searchTerm + "’ of Tcas5.2 has been discontinued in TriCast1.1, "
						+ "where it does not appear to have an equivalent locus. Therefore BeetleAtlas cannot provide any information about it.");
				}
				else
				{
					htmlBuilder.append("Gene ‘" + searchTerm + "’ of Tcas5.2 has been discontinued in TriCast1.1 and so could not be analysed here. "
							+ "However, you may inspect the raw RNAseq reads at the equivalent locus of TriCast1.1 in the ");	
					htmlBuilder.append("<a href=\"javascript:linkToUCSC2('" + gene.getNCBIid() + "','" + gene.getLocus() + "');\" "
							+ "title=\"Load RNAseq reads in UCSC browser in a new window\">UCSC Genome Browser. "
							+ "<img width=\"20\" height=\"20\" src=\"images/linkout.png\" alt=\"linkout\" class=\"linkoutImg\" /></a>"
							+ "\n");
				}
			}		
			else
			{
				...
				if(idType.equals("ncbiID") && searchTerm.length()>1 && !searchTerm.substring(0,2).equals("TC"))
				{
					htmlBuilder.append("<br />Please check the ID and that you have selected the appropriate search type.");
				}
				else if(idType.equals("ncbiID") && searchTerm.length()<2)
				{
					htmlBuilder.append("<br />Please checkthe ID and that you have selected the appropriate search type.");
				}
				else if(idType.equals("ncbiSymbol") && searchTerm.substring(0,3).equals("LOC"))
				{
					String id = searchTerm.substring(3,searchTerm.length());
					htmlBuilder.append("<br />The gene may have an alphabetic symbol. Try selecting ‘NCBI ID’ and searching with ‘" + id + "’.");
				}
				else
				{
	...
	private void buildPage(String idType, String searchTerm)
	{
		...
		// idtype radio choice 	— set in beetle2.js
		htmlBuilder.append(" • Choose search type, start entering text, then select from the autosuggest menu •<br />\n");
		...
		if(idType.equals("ncbiSymbol"))
		{
			htmlBuilder.append("<input type=\"radio\" name=\"idtype\" id=\"ncbiSymbol\" checked=\"checked\" /> Gene Symbol (e.g. Mhc, LOC660130 )<br />\n");
		}
		else
		{
			htmlBuilder.append("<input type=\"radio\" name=\"idtype\" id=\"ncbiSymbol\" /> Gene Symbol (e.g. Mhc, LOC660130)<br />\n");
		...
		if(idType.equals("product"))
		{
			htmlBuilder.append("<input type=\"radio\" name=\"idtype\" id=\"product\" checked=\"checked\" /> Gene Product (enter partial term like ‘eye’, ‘inositol’)<br />\n");
		}
		else
		{
			htmlBuilder.append("<input type=\"radio\" name=\"idtype\" id=\"product\" /> Gene Product (enter partial term like ‘eye’, ‘inositol’)<br />\n");
