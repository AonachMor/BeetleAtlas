...
{		
	...
	boolean tcQy = false;			// Flag for TC ID query as this has to be handled differently
	
	// ONLY INITIALIZED IF flyQy TRUE i.e DROSOPHILA QUERY
	String [] idList;				// array to hold Tribolium NCBI_IDs retrieved in initial Drosophila query
	...
	{	
		...
		else if(idType.equals("ncbiSymbol"))
		{
		...
		else if(idType.equals("tcID"))
		{
			geneinfoQuery = "GENEID_FROM_TC_ID";
			flyQy = false;
			prodQy = false;
			tcQy = true;
		}
		...
		if(!geneinfoQuery.equals("") && flyQy)
		{
			...
			// Initialize lists for Gene and Expression objects, same length is list of NCBI_IDs
			...
			{
				GeneSearch gs = new GeneSearch(idList[i], "ncbiID", tCat);
			...
			try 
			{
				...
				if(resSet.first())	// move to single tuple
				{
					...
					while (resSet.next())		// moves to next row while rows remain
					{
						...
						GeneSearch gs = new GeneSearch(ncbiID, "ncbiID", tCat);
		...
		// TC ID selected — first make Qy for NCBI ID then same as latter
		if(!geneinfoQuery.equals("") && tcQy)
		{
			// Make connection
			Connect cnt = new Connect();
			Connection conn = cnt.getConnection();			// Same connection conn used for all queries
			
			// Make first query for NCBI ID
			ParamQuery parNTQ = DBQuery.getParamQuery(geneinfoQuery);
			try 
			{
				parNTQ.setPrepStatement(conn);
			} 
			catch (SQLException e) 
			{System.out.println(e.toString());}
			
			String ncbiID = new String();	//	Search term for second query	
			try 
			{
				PreparedStatement prepStat = parNTQ.getPrepStatement();		
				...
				if(resSet.first())
				{
					...
					while (resSet.next())		// moves to next row while rows remain
					{	
						ncbiID = resSet.getString("NCBI_ID");
					}
				}
				try { if (resSet != null) resSet.close(); } catch (Exception e) {}; 	// added as server precaution
				try { if (prepStat != null) prepStat.close(); } catch (Exception e) {};	// added as server precaution			
			...
			// Make Second query as for NCBI ID
			String secondQuery = "INFO_FROM_GENEID";
			makeGeneInfoQuery(ncbiID, secondQuery, conn);
			
			// Check that gene has been found and if so make other queries
			if(gene != null)
			{
				// String ncbiID = gene.getNCBIid();
				makeTranscriptInfoQuery(ncbiID, conn);

				geneExpn = new GeneExpression(ncbiID);		// instantiate object to hold GeneDataset and TranscriptDataset		
				makeGeneDataQuery(ncbiID, conn);	
				makeTranscriptDataQuery(ncbiID, conn);
			}
		...
		// NCBI ID selected: make queries if valid search term and not Fly or product
		if(!geneinfoQuery.equals("") && !flyQy && !prodQy && !tcQy)
		{
			// Make connection
			Connect cnt = new Connect();
	...
	private void makeTranscriptInfoQuery(String ncbiID, Connection conn)
	{
		...
		try 
		{
			...
			if(resSet.first())
			{
				...
				{	
					...
					Transcript trans = new Transcript(transID, ncbiID);
	...
	private void makeGeneDataQuery(String ncbiID, Connection conn)
	{
		GeneTissueDataSet gDataset = new GeneTissueDataSet(ncbiID, tCat);
		...
		try 
		{
			...
			{				
				...
				while (resSet.next())		// moves to next row while rows remain
				{
					...
					GeneTissueData geneData = new GeneTissueData(ncbiID, tissueID, fpkm, repFPKMlist, sd, status);
	...
	private void makeTranscriptDataQuery(String ncbiID, Connection conn)
	{
		...
		{		
			...
			TranscriptTissueDataSet tDataset = new TranscriptTissueDataSet(ncbiID, transID);
			...
			try 
			{
				...
				if(resSet.first())
				{
					...
					{	
						...
						TranscriptTissueData transcriptData = new TranscriptTissueData(ncbiID, transID, tissueID, fpkm, sd, status);					
	...
	// retrieve Beetle NCBI_ID(s) corresponding to Drosophila FBgn or CG
