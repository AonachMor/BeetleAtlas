...
{	
	...
	public ProfileSearch(String ncbiID, GeneExpression expresssion, boolean aleProfile, boolean byPearson, double rCut, int displayMax, TissueCatalogue tCat)
	{	
		// make a ProfileTissueDataSet for the query ncbiID and retrieve the array
		ProfileTissueDataSet dataSet = new ProfileTissueDataSet(ncbiID, aleProfile);
		...
		ProfileTissueData queryData = new ProfileTissueData(ncbiID);
		for (int i = 0; i< expresssion.getGeneData().getGeneDataLength(); i++)	
		{
			if(expresssion.getGeneData().getGeneTissuedata(i) != null)
			{
				ProfileDatum datum = new ProfileDatum(expresssion.getGeneData().getGeneTissuedata(i).getNCBIid(),	
		...
		{
			GeneSearch gs = new GeneSearch(boncompList[i].getNCBIid(), "ncbiID", tCat);
			Expression express;
