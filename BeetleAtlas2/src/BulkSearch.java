...
public class BulkSearch 
{
	...
	final static String ID_TYPE = "ncbiID";	// Only NCBI-type IDs accepted
	...
	{	
		// Split search list into individual Strings and place in array (length is ncbiIDs.length)
		ncbiIDs = searchList.split(",");
	
	    // Restrict to EXPR_LENGTH (100)
	    numIDs = ncbiIDs.length;
	    ...
	    if(numIDs>0)
	    {
	    	...
	    	{
	    		GeneSearch gs = new GeneSearch(ncbiIDs[i].trim(), ID_TYPE, tCat);	// need to trim in case space at end of lines
	    		// Get expression data
	    		...
	    		{
	    			invalidIDList[invalidListSize] = ncbiIDs[i];
					invalidListSize++;
					invalidIDs++;	
	    		}
	    	}
