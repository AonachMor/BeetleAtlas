...
function sendSearchGeneForm() 
{
	...
	{ 
		...
		if (idtype=="ncbiID")
		{
			var ncbiURL = "https://motif.mvls.gla.ac.uk/BeetleAtlas2/?search=gene&gene=" + gene + "&idtype=ncbiID";
			window.open(ncbiURL,'_self');
		}
		else
		{		
...
function sendSearchProfileForm() 
{
	...
	else
	{
		...
		hiddenMaxField.setAttribute("value", maxdisplayed);
		form.appendChild(hiddenMaxField);
		// include Embryo in profile?
/*		var includeE = document.getElementById('rcut').value;
		var includeEHidden = document.createElement("input");
		includeEHidden.setAttribute("type", "hidden");
		includeEHidden.setAttribute("name", "includeE");
		if(document.getElementById('include').checked)
		{
			includeEHidden.setAttribute("value", "include");
		}
		else
		{
			includeEHidden.setAttribute("value", "exclude");
		}
		form.appendChild(includeEHidden);*/
...
// determines name of gene and auto-assigns geneID if TC...
function getIDType()
{
	...
	else if(input.substring(0,4) == "FBgn")
	{
		idtype = "flyFBgn"		
	}
...
// for Profile search need to handle this differently
function getProfileIDType()
{
	var idtype;		// idtype choice
	var input = document.getElementById('inputField').value;	
	if(input.substring(0,2) == "TC")
...
function profileKey(e)
{
	...
	// link to iBeetle for gene in separate window
	...
	function linkToUCSC(id, locus)
	{
		var url = "/BeetleBrowse/index.html?id=" + id + "&locus=" + locus;
		var w = 720;
		var h = 560;
	...
	// Send to BeetlePara to generate List/Button window	
	function listParalogues(id) 
	{
		var url = "/BeetlePara/index.html?id=" + id + "&type=para";
		var w = 460;
		var h = 600;
	...
	function listOrthologues(id) 
	{
		var url = "/BeetlePara/index.html?id=" + id + "&type=ortho";
		var w = 460;
		var h = 600;
