...
public class TissueCatalogue
{
	...
	private int DOUB_LIST_LEN = 100;		// length of array (currently only needs 14, so should be ok for a while)
	...
	private int DEV_LIST_LEN = 100;			// length of array (currently only needs 14, so should be ok for a while)
	...
	public TissueCatalogue()
	{
		...
		//Debugging
/* 		for(int x=0; x<tissueListSize; x++)
		{
			System.out.println(tissueList[x].toString());
		}*/
	}
	...
	{	
		...
		{	
		    ...
		    try { if (stmt != null) stmt.close(); } catch (Exception e) {};		// added for server precaution
		}
		...
		populateDoubletList(); 
		populateUniList(); 
/*		for(int x=0; x<devListSize; x++)
		{
			System.out.println(devList[x].toString());
		}*/
	}
	...
	private void populateDoubletList()
	{
		for(int i=0; i<doubletDisplaysSize; i++)
		{
			...
			doubletListSize++;  
				//System.out.println(doublet.toString());		// for debugging
		}
	}
    ...
    // Sorts tissueList in alphabetical order for pulldown menu of tissues in Category and Top queries
    public void sortTissueList()
    {
    	Tissue lowest;		// holder for TissueDoublet with lowest value
		int lowestPos;		
		for (int i=0; i<tissueListSize-1; i++)		// do series of runs
		{
			for (int j=i+1; j<tissueListSize; j++)	// for each run process list
			{
				lowest = tissueList[i];				// first of unsorted assigned to lowest
				lowestPos = i;

				if(lowest.getTissueName().compareTo(tissueList[j].getTissueName()) > 0)
				{
					lowestPos = j;
				}
				
				lowest = tissueList[lowestPos];
				tissueList[lowestPos] = tissueList[i]; 		// shift current first
				tissueList[i] = lowest;					// replace
			}
		}   	
    }
	...
	private void populateDoubletDisplayList()
	{
		...
		try
		{
					...
					try 
					{
						...
						if(resSet.first())	// move to single tuple
						{
							uniTissName = resSet.getString("UniTissueName");
							 //System.out.println("uniTissName: " + uniTissName);
						}
					    try { if (resSet != null) resSet.close(); } catch (Exception e) {}; 	// added as server precaution
			...
			//System.out.println("doubletDisplaysSize: " + doubletDisplaysSize);
/*			for(int x=0; x<doubletDisplaysSize; x++)
			{
				System.out.println(doubletDisplays[x].getDisplayPos() + ", " + doubletDisplays[x].getUniTissueID() + ", " + doubletDisplays[x].getUniTissueName());
			}*/
		}
