...
public class BeetleAtlas extends HttpServlet 
{
	...
	{	
		...
		{		
			...
			String idType = req.getParameter("idtype");						// Parameter to specify whether GeneID, GeneSymbol etc)
			...
			else if(idType.equals("geneName") )	// Product — redundant
			{
				Gene[] geneList = search.getGeneList(); 				
				Expression[] expressList = search.getExpressList();
				int listSize = search.getListSize();
		...
		else if (req.getParameter("search").equals("profile"))
		{
			...
			boolean aleProfile = false; 	// Include embryo data in profile search. Option removed — DO NOT INCLUDE 
			/*	if(req.getParameter("includeE").equals("include"))
				{
					aleProfile = true;
				}
			*/
			
			// get GeneExpression object and geneID corresponding to geneQuery
			...
			if(expression != null)
			{
				geneID = geneSearch.getGene().getGeneID(); 
			}
			else		// abort
			{
				ProfilePage profilePage = new ProfilePage (geneQuery, idType, geneID, null, null, null, null, null, displayMax, 
						tCat, byPearson, aleProfile, rString, includeErrors, showWhole);
	    		writer.println(profilePage.getHTML());
				return;
			}
			
			ProfileSearch search = new ProfileSearch(geneID, expression, aleProfile, byPearson, rCut, displayMax, tCat);
			Expression[] expressList = search.getExpressList();
			Gene[] geneList = search.getGeneList();	
			ProfileTissueData[] profileDataList = search.getDataList();
			
			ProfilePage profilePage = new ProfilePage (geneQuery, idType, geneID, searchGene, expression, expressList, geneList, profileDataList, displayMax, 
					tCat, byPearson, aleProfile, rString, includeErrors, showWhole);
