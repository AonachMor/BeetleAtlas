...
public class ProfileTissueDataSet 
{
	...
	{
		String query = new String();		// GeneID, FPKM, Status, TissueID for ALL genes
		if(aleProfile)
		{
		...
		try 
		{
			...
			if(resSet.first())
			{
				...
				{	
					...
					{
						// checks if new set by by initial value of atStart or change in value of geneID (String 1)
						if(atStart || !resSet.getString(1).equals(geneID))
						{
							...
							// read this first geneID and use it to construct the new GeneExCorrelation
							geneID = resSet.getString("GeneID");						
							data = new ProfileTissueData(geneID);
							...
							ProfileDatum datum = new ProfileDatum(geneID, fpkm, status, tissueID);
							data.addDatum(datum);
