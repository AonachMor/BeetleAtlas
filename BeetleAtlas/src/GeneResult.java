/*
	...
	Updated: 29.08.2024 (UCSC Browser link reinstated. Works, but the loci values seem wrong)
	Last Update:20.05.2025 Altered presentation of linkout to UCSC Browser
*/
...
public class GeneResult 
{
	...
	private String getAdultLarvalResults(String geneTableID, String transTableID, GeneTissueDataSet dataset)
	{
		...
		sb.append("<a href=\"javascript:linkToUCSC('" + gene.getGeneID() + "','" + gene.getLocus() + "');\" "
    ...
    public String getEGeneTableRow(GeneExpression expn, Gene gene, TissueCatalogue tCat)
    {
    	...
    	StringBuilder sb = new StringBuilder("<tr><td><span class=\"mobileHide\">" + gene.getGeneID() + "</span></td>");	// geneID in first column
	...
	private String getALTranscriptTableRows(GeneExpression expn, Gene gene, TissueCatalogue tCat)
	{
		...
		for (int i=0; i<expn.getTranscriptDataSize(); i++)
		{
			...
			String transcriptID = ttds.getTranscriptID();
			// TODO Split transcriptID into geneID and suffix (-RA etc.) and hide former from mobiles.
	...
	private String getSVG(TissueCatalogue tCat)
	{
		...
		ImageCreator ic = new ImageCreator(gene.getGeneID(), tCat, fpkms, errors, false);
		return ic.getSVG();
	}
